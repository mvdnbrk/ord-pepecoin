...
Trait values may be **strings**, **booleans**, **integers**, or **null**. Other CBOR types (floats, bytes, arrays, maps) are rejected — the entire `properties` tag is ignored if any trait value uses an unsupported type. Trait names must be unique — duplicate names invalidate the `properties` tag.
