...
- **Key:** `"metadata"` or `"metadata;br"` (Brotli-compressed)
...
Indexers should decode valid CBOR and store the raw bytes. Invalid CBOR is ignored — the inscription remains valid without metadata. See [Compression](#compression) for `"metadata;br"` handling.
...
- **Key:** `"properties"` or `"properties;br"` (Brotli-compressed)
- **Value:** CBOR-encoded bytes ([RFC 8949](https://www.rfc-editor.org/rfc/rfc8949.html))
