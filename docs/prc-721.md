...
For example, a `title` in `"properties"` is displayed as a subtitle on the inscription page. With `"metadata"`, the indexer would have to guess which field is the title.

- **Key:** `"properties"` (10 bytes)
- **Value:** CBOR-encoded bytes ([RFC 8949](https://www.rfc-editor.org/rfc/rfc8949.html))
...
| Key | Type | Description | Example |
|-----|------|-------------|---------|
| `title` | string | Inscription title (displayed as subtitle) | `"Rare Pepe #42"` |
| `traits` | map\<string, string\> | Collection traits for filtering | `{"background": "gold", "eyes": "laser"}` |
...
This mirrors upstream ordinals' distinction between Tag 5 (metadata — arbitrary user data) and Tag 17 (properties — protocol-recognized attributes like title and traits).
